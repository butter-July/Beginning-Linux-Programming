...
static int enter_new_entry(cdc_entry *entry_to_update)
{
    cdc_entry new_entry;
    char tmp_str[TMP_STRING_LEN+1];
    ...
    strip_return(tmp_str); //不要换行符，所以去掉
...
//删除一个标题项，如果标题项被删除了那么原来属于她的曲目信息也会被删除
static void del_cat_entry(const cdc_entry *entry_to_delete) {
    int track_no=1;
    int delete_ok;
...
static cdc_entry find_cat(void) {
    ...
    do {
        ...
        if (strlen(tmp_str) > CAT_CAT_LEN) {
            fprintf(stderr,"Sorry string too long maxmum %d  characters\n",CAT_CAT_LEN);
            string_ok=0;
        }
    }while (!string_ok);
    while(!entry_selected) {
        item_find=searvh_cdc_entry(tmp_str,&first_call);
        if (item_find.catalog[0]=='\0') {
            any_entry_found=1;
            printf("\n");
            display_cdc(&item_find);
            ...
            }else printf("Soorry ,nothing found");
        }
        return(item_find);
    }
}
...
static void count_all_entries(void) {
    ...
    do {
        cdc_found=searvh_cdc_entry(search_string,&first_time);
        if (cdc_found.catalog[0]) {
            cd_entries_found++;
            tracks_no++;

            do {
                cdt_found=get_cdt_entry(cdc_found.catalog,tracks_no);
...
//删除字符串尾部的换行符，Linux 使用一个单独的换行符来表明一行的结束
static void strip_return(char *string_to_strip ) {
    int len ;
        ...
        switch (c) {
            case 'i':
                if (!database_initialize(1)) {
                    result = EXIT_FAILURE;
                    fprintf(stderr,"Failed to initilize database\n");
                }
                break;
            case ':':
            case '?':
